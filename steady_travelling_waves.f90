...
!!-----------------------------------------------------------------------
      ...
      !!
      !! IMPORTANT: in Antuono (2022) some velocity scales are defined in an ambiguous way! <<<<<<<<<<<
      !! In particular, the scale u0 = eps * sqrt(grav*z0) has to be associated ONLY to the Drift velocity
      !! and to the pressure field. Further, the scale w0=u0 has no application at all.
      !! Conversely, the velocity components have to be scaled by "sqrt(grav*z0)*Froude" 
      !!
      wave_sol%eta  = eps * z0 * wave_sol%eta
      IF (flag_no_drift) THEN                                    
         wave_sol%u_fs = sqrt(grav*z0) * Froude * wave_sol%u_fs + v_current - abs(U_drift)   !! WITHOUT drift
      ELSE   
         wave_sol%u_fs = sqrt(grav*z0) * Froude * wave_sol%u_fs + v_current                  !! WITH drift
      ENDIF 
      wave_sol%w_fs = sqrt(grav*z0) * Froude * wave_sol%w_fs
      ...
      do i=1,nsample
        ...
        do j=1,nsample_z
          ...
          wave_sol%u(i,j) = vel_sol(1) * sqrt(grav*z0) * Froude 
          wave_sol%w(i,j) = vel_sol(2) * sqrt(grav*z0) * Froude 
 
          !! dimensionless pressure (using the Bernoulli equation)
          !! pressure scale: p0 = rho0 * u0**2
